from torch.fft import fftshift as fftshift, fftn as fftn_t, ifftn as ifftn_t, ifftshift as ifftshift
...
def th_ff_sampling_3d(field):
    """FFT routine for centered data"""
    return th_fft.fftshift(
        th_fft.fftn(th_fft.ifftshift(field, dim=(-1, -2,-3)), norm="ortho"), dim=(-1, -2 ,-3)
    )
...
def th_iff_sampling_3d(field):
    """IFFT routine for centered data"""
    return th_fft.fftshift(
        th_fft.ifftn(th_fft.ifftshift(field, dim=(-1, -2,-3)), norm="ortho"), dim=(-1, -2 ,-3)
    )
...
def get_pad_size(shape,sampling_init,sampling_desired):
    ...
    return ((-1/2)*(shape*(1-sampling_init/sampling_desired))).int().repeat_interleave(2)


def Bulk_Resample(X,padding):
    return Pad(th_iff_sampling(Pad(th_ff_sampling(X),tuple(padding.tolist()))),tuple((-1*padding).tolist()) )


def Bulk_Resample_3d(X,padding):
    return Pad(th_iff_sampling_3d(Pad(th_ff_sampling_3d(X),tuple(padding.tolist()))),tuple((-1*padding).tolist()) )
...
def shear(array,axis,magnitude):
    ...
    coord_1_ax_val = th.arange(array.shape[coord_1_ax])- int(array.shape[coord_1_ax]//2)
    coord_2_ax_val = th.arange(array.shape[coord_2_ax])- int(array.shape[coord_2_ax]//2)
    ...
    sheared =  sheared*th.exp(-2j*th.pi*freqs* (magnitude[0]*coord_1_ax_val+magnitude[1]*coord_2_ax_val ))
...
def shift(sample,shift):
    ...
    if len(sample_shape) ==2:
        ...
        freq_x, freq_y = th.meshgrid(th.fft.fftfreq(sample_shape[0], 1), th.fft.fftfreq(sample_shape[1], 1),indexing = 'ij')
        sample = th.fft.fftn(sample,dim = (-1,-2),norm = 'ortho')
        sample = sample* th.exp(-2j*th.pi * (freq_x*shift[0]+freq_y*shift[1]))
        sample = th.fft.fftn(sample,dim = (-1,-2),norm = 'ortho')

    else:
        freq_x, freq_y,freq_z = th.meshgrid(th.fft.fftfreq(sample_shape[0], 1), th.fft.fftfreq(sample_shape[1], 1),th.fft.fftfreq(sample_shape[2], 1),indexing = 'ij')
        sample = th.fft.fftn(sample,dim = (-1,-2,-3),norm = 'ortho')
        sample = sample* th.exp(-2j*th.pi * (freq_x*shift[0]+freq_y*shift[1]+freq_z*shift[2]))
        sample = th.fft.fftn(sample,dim = (-1,-2,-3),norm = 'ortho')
