...
def iff(a):
    return fftshift(nfft.ifft2(ifftshift(a), norm="ortho"))
...
class Field:
    ...
    def ASM(self, z, inplace=False, F_up=False, f=None):
        """
        Angular spectrum propogation
        ...
        Hz = np.exp(
            (2j * np.pi * z) * (np.sqrt(1 / (self.wavel) ** 2 - fxx ** 2 - fyy ** 2))
        )

        if inplace:
            self.field = iff(ff(self.field) * Hz)
        else:
            res = self.copy()
            res.field = iff(ff(res.field) * Hz)
            return res
