...
def shear(array,axis,magnitude):
    ...
    freqs = th.fft.fftfreq(sample.shape[axis], 1)[slice_freq]
...
def rotate(array,axis,angle):
    ...
    if axis ==0:
        ...
        X = shear(array,(axis+1)%3,(0,np.tan(theta/2)))
        X = shear(X,(axis-1)%3,(0,-np.sin(theta)))
        return shear(X,(axis+1)%3,(0,np.tan(theta/2)))    
    elif axis ==1:
        X = shear(array,(axis+1)%3,(np.tan(theta/2),0))
        X = shear(X,(axis-1)%3,(0,-np.sin(theta)))
        return shear(X,(axis+1)%3,(np.tan(theta/2),0))
    
    elif axis==2:

        X = shear(array,(axis+1)%3,(np.tan(theta/2),0))
        X = shear(X,(axis-1)%3,(-np.sin(theta),0))
        return shear(X,(axis+1)%3,(np.tan(theta/2),0))
